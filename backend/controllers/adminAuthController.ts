...
export const loginAdmin = async (req: Request, res: Response) => {
  ...
  const memberships: any[] = await prisma.$queryRawUnsafe(
    'SELECT * FROM usermembership WHERE userId = ?',
    user.id
  );

  const isSuperAdmin = !!user.isSuperAdmin;
  const adminMembership = memberships.find((m: any) => ['OWNER', 'ADMIN'].includes(m.role));
  ...
  if (!role) throw new AppError('User does not have admin access.', 403);
