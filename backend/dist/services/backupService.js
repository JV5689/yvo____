...
export const generateBackup = async (jobId) => {
    ...
    try {
        ...
        const updatedRecordCounts = typeof job.recordCounts === 'object' && job.recordCounts !== null && !Array.isArray(job.recordCounts) ? { ...job.recordCounts } : {};
        for (const moduleName of modulesToExport) {
            const modelDelegateString = MODULE_MODELS[moduleName];
            if (!modelDelegateString)
                continue;
            await prisma.backupJob.update({
                where: { id: jobId },
                data: {
            ...
            let query = { companyId: company.id };
            if (job.filters?.dateRange?.start || job.filters?.dateRange?.end) {
                ...
                if (job.filters.dateRange.start)
                    query[dateField].gte = new Date(job.filters.dateRange.start);
                if (job.filters.dateRange.end)
                    query[dateField].lte = new Date(job.filters.dateRange.end);
            }
            if (moduleName === 'users') {
                query = { memberships: { some: { companyId: company.id } } };
            }
            let includeArgs = undefined;
            ...
            const modelDelegate = prisma[modelDelegateString];
